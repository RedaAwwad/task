<script setup lang="ts">
  ...
  const btnSizes: Record<Sizes, string> = {
    ...
    base: `font-medium text-sm px-6 py-2 ${
...
<template>
  <button
    class="inline-block"
    :disabled="loading"
    :class="[btnVariants[variant], btnSizes[size]]"
    @click="handleClickEvent"
  >
    <div v-if="loading" class="px-3 h-full flex items-center justify-center">
      <span>
        <svg
          class="w-[20px] h-[20px] text-gray-200 animate-spin dark:text-gray-600 fill-secondary"
          viewBox="0 0 100 101"
          fill="none"
